...
#include "../../FKCW_Depend/freetype2/android/prebuilt/libfreetype2/include/ft2build.h" 
#include "../../FKCW_Depend/freetype2/android/prebuilt/libfreetype2/include/freetype/freetype.h" 
#include "../../FKCW_Depend/freetype2/android/prebuilt/libfreetype2/include/freetype/ftstroke.h" 
#endif

#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS) 
#include FT_FREETYPE_H
#include FT_STROKER_H
#include "../../FKCW_Depend/freetype2/ios/libfreetype2/ft2build.h"  
#endif
...
public:
	...
	// 缓冲盒是全部渲染批次的最大包围盒，所以可能大于当前的渲染批次包围盒
...
//-------------------------------------------------------------------------
// 位图字体数据
class FKCW_Font_Info
{
public:
	static FKCW_Font_Info* create_font(FT_Library library, const char* fontname, FT_UInt width_pt, FT_UInt height_pt, FT_UInt ppi=72);
	static FKCW_Font_Info* create_font(FT_Library library, const char* fontname, FT_Long face_idx, FT_UInt width_pt, FT_UInt height_pt, FT_UInt ppi);

public:
	void		release();
	FKCW_Font_Info*	add_pass(const RenderPassParam& param);
	...
	FKCW_Font_Info*	add_hackfont(const char* fontname, std::set<FT_ULong>* charset, FT_UInt shift_y = 0);
	FKCW_Font_Info*	add_hackfont(const char* fontname, FT_Long face_idx, std::set<FT_ULong>* charset, FT_UInt shift_y);
...
private:
	...
	FT_Face		m_face;
	bool		m_has_kerning;
	GlyphRenderer*			m_ob_renderer;			// 留给外界的观察者渲染器
	GlyphRenderer*			m_private_renderer;		// 内部渲染器
	std::set<FT_ULong>*		m_available_charset;
	std::vector<FKCW_Font_Info*>	m_hackfonts;
};