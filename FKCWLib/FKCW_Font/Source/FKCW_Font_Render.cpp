...
//-------------------------------------------------------------------------
	...
	FT_Error GlyphRenderer::render(FT_Glyph& glyph, IBitmap** pbuf, FT_Vector* top_left_pixel, FT_Vector* advance_pixel)
	{
		...
		if ( buf == NULL )
		{
			buf = new Bitmap_32bits( ((bbox.xMax - bbox.xMin) >> 6) + 2*FKCW_Font_Bitmap_Padding, ((bbox.yMax - bbox.yMin) >> 6) + 2*FKCW_Font_Bitmap_Padding, FKCW_Font_Bitmap_Padding );
			*pbuf = buf;
		}
	...
	//-------------------------------------------------------------------------
	FKCW_Font_Info* FKCW_Font_Info::create_font(FT_Library library, const char* fontname, FT_UInt width_pt, FT_UInt height_pt, FT_UInt ppi/*=72*/)
	{
		return create_font(library, fontname, 0, width_pt, height_pt, ppi);
	}
	//-------------------------------------------------------------------------
	FKCW_Font_Info* FKCW_Font_Info::create_font(FT_Library library, const char* fontname, FT_Long face_idx, FT_UInt width_pt, FT_UInt height_pt, FT_UInt ppi)
	{
		if ( library && fontname)
		{
			FKCW_Font_Info* f = new FKCW_Font_Info(library);
	...
	//-------------------------------------------------------------------------
	void FKCW_Font_Info::release()
	{
		delete this;
	}
	//-------------------------------------------------------------------------
	FKCW_Font_Info* FKCW_Font_Info::add_pass(const RenderPassParam& param)
	{
		if ( renderer() == NULL )
		{
	...
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::render_charcode(FT_ULong char_code, GlyphBitmap* bitmap, FT_UInt prev_idx)
	{
	...
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::get_char_index(FT_ULong charcode)
	{
	...
	//-------------------------------------------------------------------------
	bool FKCW_Font_Info::render_charidx(FT_UInt char_idx, GlyphBitmap* bitmap, FT_UInt prev_idx)
	{
	...
	//-------------------------------------------------------------------------
	// has kerning info
	bool FKCW_Font_Info::has_kerning()
	{
		return m_has_kerning;
	}
	//-------------------------------------------------------------------------
	FT_Vector FKCW_Font_Info::get_kerning(FT_UInt left_idx, FT_UInt right_idx)
	{
		FT_Vector delta; 
		delta.x = 0;
		delta.y = 0;
	...
	//-------------------------------------------------------------------------
	const char* FKCW_Font_Info::font_name()
	{
		return m_fontname.c_str();
	}
	//-------------------------------------------------------------------------
	FT_Library FKCW_Font_Info::library()
	{
		return m_library;
	}
	//-------------------------------------------------------------------------
	bool FKCW_Font_Info::is_bitmap()
	{
		return m_isbitmap;
	}
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::char_width_pt()
	{
		return m_char_width;
	}
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::char_height_pt()
	{
		return m_char_height;
	}
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::ppi()
	{
		return m_ppi;
	} 
	//-------------------------------------------------------------------------
	void FKCW_Font_Info::set_shift_y(FT_UInt sy)
	{
		m_shift_y = sy;
	}
	//-------------------------------------------------------------------------
	FT_UInt FKCW_Font_Info::extend_pt()
	{
		return m_extend_pt;
	}
	//-------------------------------------------------------------------------
	FT_Short FKCW_Font_Info::underline_position()
	{
		return m_underline_position - m_extend_pt;
	}
	//-------------------------------------------------------------------------
	FT_Short FKCW_Font_Info::underline_thickness()
	{
		return m_underline_thickness;
	}
	//-------------------------------------------------------------------------
	FT_Face FKCW_Font_Info::face()
	{
		return m_face;
	}
	//-------------------------------------------------------------------------
	void FKCW_Font_Info::set_available_charset(std::set<FT_ULong>* charset)
	{
		m_available_charset = charset;
	}
	//-------------------------------------------------------------------------
	FKCW_Font_Info* FKCW_Font_Info::add_hackfont(const char* fontname, std::set<FT_ULong>* charset, FT_UInt shift_y /*= 0*/)
	{
		return add_hackfont(fontname, 0, charset, shift_y);
	}
	//-------------------------------------------------------------------------
	FKCW_Font_Info* FKCW_Font_Info::add_hackfont(const char* fontname, FT_Long face_idx, std::set<FT_ULong>* charset, FT_UInt shift_y)
	{
		FKCW_Font_Info* hackfont = NULL;
		if( m_fontname.empty() )
		{
			hackfont = FKCW_Font_Info::create_font(library(), fontname, face_idx, char_width_pt(), char_height_pt(), ppi());
		}
		else
		{
			hackfont = FKCW_Font_Info::create_font(library(), m_fontname.c_str(), face_idx, char_width_pt(), char_height_pt(), ppi());
		}
		if ( !hackfont )
		{
			return NULL;
	...
	//-------------------------------------------------------------------------
	FT_Error FKCW_Font_Info::_render_ready_char(GlyphBitmap* bitmap)
	{
		FT_Error error;
		FT_Glyph glyph = NULL;
	...
	//-------------------------------------------------------------------------
	FT_GlyphSlot FKCW_Font_Info::current_glyph()
	{
		return m_face->glyph;
	}
	//-------------------------------------------------------------------------
	bool FKCW_Font_Info::load_glyph_from_index(FT_UInt char_idx)
	{
		return 0 == FT_Load_Glyph(m_face, char_idx, FT_LOAD_DEFAULT);
	}
	//-------------------------------------------------------------------------
	bool FKCW_Font_Info::load_glyph_from_char(FT_ULong char_code)
	{
		FT_UInt char_idx = get_char_index(char_code);
	...
	//-------------------------------------------------------------------------
	void FKCW_Font_Info::set_renderer(GlyphRenderer* _renderer)
	{
		m_ob_renderer = _renderer;
	}
	//-------------------------------------------------------------------------
	GlyphRenderer* FKCW_Font_Info::renderer()
	{
		return m_ob_renderer;
	}
	//-------------------------------------------------------------------------
	FT_Error FKCW_Font_Info::init(FT_Library& lib, const char* fontname, FT_Long face_idx, FT_UInt width_pt, FT_UInt height_pt, FT_UInt ppi)
	{
		FT_Error error = 0;
	...
	//-------------------------------------------------------------------------
	FKCW_Font_Info::FKCW_Font_Info(FT_Library lib) 
