//--------------------------------------------------------------------
#include "../Include/FKCW_Base_RichLabel.h"
//--------------------------------------------------------------------
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_WIN32
FKCW_Base_RichLabel::FKCW_Base_RichLabel()
{
...
CCSize FKCW_Base_RichLabel::MeasureRichString(const char* pText,
...
bool FKCW_Base_RichLabel::InitWithRichStringShadowStroke(const char * pText,
														 int         nWidth,
														 int         nHeight,
...
typedef struct Span {
	...
	// 颜色的子标签（支持渐变色，color专用）
	int color;
	int toColor;
	float duration;
	bool transient;

	int fontSize;							// 字体大小（font类型专用）
	char* fontName;							// 字体名称（font专用）
...
typedef struct {
	...
	float			elapsedTime;
	unsigned char*  data;
	bool			sizeOnly;					// 该值为真则表示当前仅在做测量
	bool			needTime;					// 该值为真则表示Label有颜色变化效果，需要帧更新
} tImageInfo;

// [NSString, UIImage]的字典
...
FKCW_Base_RichLabel::FKCW_Base_RichLabel() :
	m_tagShadowStrokePadding(CCPointZero),
	m_nRealLength(0) {
}
		...
		DECRYPT_FUNC decryptFunc) 
{
	...
	if (!_initWithString(pText, eAlignMask, pFontName, nSize, &info, 
		&m_tagShadowStrokePadding, &m_vecLinkMetas, &m_vecImageRects, decryptFunc))
	{
	   return false;
	}
	...
	m_pData = info.data;
	m_nRealLength = info.realLength;
	m_bIsNeedUpdate = info.needTime;
											...
											DECRYPT_FUNC decryptFunc) 
{
	...
	if (!_initWithString(pText, kAlignCenter, pFontName, nSize, &info,
		NULL, NULL, NULL, decryptFunc)) {
		return CCSizeZero;
	}
